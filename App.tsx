...
const App: React.FC = () => {
  ...
  const { isKeyReady, promptForApiKey, handleApiError, getGenAiClient } = useApiKey(isLoggedIn);
  ...
  if (!isKeyReady) {
    return <ApiKeyPrompt promptForApiKey={promptForApiKey} />;
  }
       ...
       <header className="bg-slate-900/70 backdrop-blur-sm sticky top-0 z-10 py-4 px-4 sm:px-8 border-b border-slate-800">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-slate-100">GenMedia</h1>
          </div>
          <button 
            onClick={handleLogout} 
            className="bg-slate-700 hover:bg-slate-600 text-slate-300 hover:text-white font-semibold py-2 px-4 rounded-lg transition duration-300 text-sm">
            Logout
          </button>
        </div>
      </header>
